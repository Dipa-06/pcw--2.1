...
public class StudentList {
    ...
    private static void displayRandomStudent() {
        ...
        try (BufferedReader reader = new BufferedReader(new FileReader(FILE_NAME))) {
            String[] studentNames = reader.readLine().split(",");
            System.out.println(studentNames[new Random().nextInt(studentNames.length)].trim());
        } catch (IOException exception) {
            System.out.println("Error reading student data.");
        }
        System.out.println("Data Loaded.");
    ...
    private static void addNewStudent(String studentName) {
        System.out.println("Loading data ...");
        try (BufferedWriter writer = new BufferedWriter(new FileWriter(FILE_NAME, true))) {
            writer.write(", " + studentName);
            writer.newLine();
            writer.write("List last updated on " +
    ...
    private static void searchStudentByName(String studentToFind) {
        System.out.println("Loading data ...");
        try (BufferedReader reader = new BufferedReader(new FileReader(FILE_NAME))) {
            for (String student : reader.readLine().split(",")) {
                if (student.trim().equals(studentToFind)) {
    ...
    private static void countWordsInFile() {
        System.out.println("Loading data ...");
        try (BufferedReader reader = new BufferedReader(new FileReader(FILE_NAME))) {
            System.out.println(reader.readLine().trim().split("\\s+").length + " word(s) found");
        } catch (IOException exception) {
            System.out.println("Error reading student data.");
        }
        System.out.println("Data Loaded.");
